...
Every chapter in this book ends with \term{recipe} that walks through all the code for a certain giza demo.  Each recipe has a very simple page layout, as seen in Figure~\ref{fig:Recipe1}.
...
\item Call \code{COMMON.endFrame} at the end of the draw cycle so that another animation frame is always requested, unless a WebGL error occurs.
\item After defining the draw function, call it once (passing 0 for \code{currentTime}) to kick off the infinite animation.
\end{enumerate}
