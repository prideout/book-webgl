...
This chapter's recipe is called \code{StrobeLight}, which isn't too exciting but at least establishes some boilerplate we'll be using for future recipes.  First let's take a look at its HTML; see Listing~\ref{lst:StrobeLight1}.
...
While giza has no dependencies on any JavaScript libraries, the COMMON layer depends on the following third-party libraries to make life a little easier:
...
COMMON.endFrame = ...; // see Listing~\ref{lst:COMMON:endFrame}.
...
var main = function() {
  ...
  GIZA.init(); //  see Listing~\ref{lst:GIZA:init1} and \ref{lst:GIZA:init2}
