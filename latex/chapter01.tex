...
This is not just a book, it's a library.  Many of the code listings on these pages are lifted directly from the giza library, named after the Egyptian city known for its pyramids.  The pyramids form a skyline of triangles, which are the fundamental drawing primitive in computer graphics.  The pyramid shape also describes a \term{viewing frustum}, a spatial region that encompasses everything within a certain vantage point in a 3D graphics program.

As a design philosphy, giza never calls methods on the WebGL context object -- that's your job!  This differentiates giza from higher-level libraries such as ThreeJS.  Instead of providing an abstraction of WebGL, giza is utility layer; it provides help with vector math, constructing buffers of mesh data, and so on.
...
The first line is repeated at the top of all giza files.  This allows clients to include a subset of giza because it creates a new \code{GIZA} namespace only if it doesn't already exist.  This isn't necessary when using giza in minified form, since the entire library is included in that case.

Other than \code{GIZA}, the only global variables that we set are \code{COMMON} and \code{gl}.  The \code{gl} variable is the context object that exposes the entire WebGL API, which we'll learn about in Section~\ref{sec:context}.  We made it a global for terseness, since we make a huge number of calls to the context object.
...
There's nothing wrong with the above approach for simple applications.  It's common, however, to use CSS to specify a dynamic display area (e.g., \code{width:100\%}).  Moreover CSS pixels don't necessarily correspond to actual device pixels.  This is especially true on displays with a high pixel density, where browsers typically upscale CSS pixels by 2x.

\begin{sidenote}%
If you assume that device pixels are 1:1 with CSS pixels, you might see bluriness in your WebGL canvas due to upscaling.
\end{sidenote}
