...
\subsubsection{Function Declaration Syntax}

In this book, we define all functions using function expressions rather than function statements.  In other words, we prefer this syntax:

\begin{lstlisting}[language=JavaScript]
var main = function() {
  console.info("This is a function expression!");
};
\end{listlising}
...
\begin{lstlisting}[language=JavaScript]
function main() {
  console.info("This is a function declaration!");
};
\end{listlising}

They mean almost the same thing, but there are subtle differences.  Douglas Crockford (known for JSON and \emph{JavaScript: The Good Parts}) recommends expressions over statements because they emphasize the first-class nature of functions in JavaScript, and they avoid the scope-hoisting behavior that characterizes function declarations.
...
\begin{lstlisting}[
    caption={Adjust the canvas size and create a WebGL context.},
...
In addition to requesting the next animation cycle, giza's draw wrapper also checks the WebGL error state and halts animation if an error occurs.  This is good practice to prevent cascading errors and maintain clean code.
...
var COMMON = (function () {
  ...
  // Load the scripts asynchronously and execute the recipe-specific main() function.
