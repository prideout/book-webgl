...
Other than \code{GIZA}, the only global variable that we set is \code{COMMON}.  This is where we place a small handful of WebGL helpers that can be called from any of our sample apps.  Unlike methods in \code{GIZA} (which has no dependencies on third-party libraries), methods in \code{COMMON} are allowed to call jQuery functions and WebGL methods.
...
The github project also contains the code for all the samples in this book, including HTML and CSS source (and the \code{COMMON} layer).
...
\begin{enumerate}
\item Call \code{GIZA.init()} and fetch the context object that it extracted from the \code{<canvas>} element.
