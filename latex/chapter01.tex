...
Other than \code{GIZA}, the only global variable that we create is \code{COMMON}.  This is where we place a small handful of WebGL helpers that can be called from any of our sample apps.  Unlike methods in \code{GIZA} (which have no dependencies on third-party libraries), methods in \code{COMMON} are allowed to call jQuery functions and WebGL methods.
...
The \code{clear} command resets all the pixels in the canvas, while \code{clearColor}, \code{clearDepth}, and \code{clearStencil} are simply configuring the WebGL state machine.  Most of the WebGL API consists of state-setting commands; in fact there are only three methods in the entire API that can actually paint pixels:
