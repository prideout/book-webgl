...
No matter which browser you use during development, your best friend will quickly become your browser's \term{Development Console}.  Find it and get acquainted if you're not already.  In our sample code, we tend to print errors using JavaScript's \code{console} object rather than calling the \code{alert} function, and these messages are only visible in the development console.
...
HeadJS enables us to simplify our HTML by sharing a list of JavaScript libraries in \code{common.js} instead of duplicating a large set of \code{<script>} tags in every HTML file.  It also has the benefit of loading scripts asynchronously for better performance.  See Listing~\ref{lst:COMMON:file} for \code{common.js}.
