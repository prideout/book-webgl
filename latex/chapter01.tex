...
Note that \code{requestAnimationFrame} is actually more like \code{setTimeout} than \code{setInterval} because it doesn't automatically repeat itself.  Our \code{COMMON} layer provides a function that can be called at the end of your draw function to request the next animation cycle.  It also checks the WebGL error state, and halts animation if an error occurs.  This is good practice to maintain clean, error-free code.  See Listing~\ref{lst:COMMON:endFrame}.
...
HeadJS enables us to simplify our HTML by sharing a list of JavaScript libraries in \code{common.js}, instead of duplicating a large set of \code{<script>} tags in every HTML file.  It also has the benefit of loading scripts asynchronously for better performance.  See Listing~\ref{lst:COMMON:file} for \code{common.js}.  We'll extend this file in future chapters by adding some small utilities for loading textures, compiling shaders, etc.
...
By now you should understand everything in Listing~\ref{lst:StrobeLight2}, but just in case here's what's happening:

\begin{enumerate}
\item Call \code{GIZA.init()} and create an alias to the context object that was extracted from the \code{<canvas>} element.
\item Define a draw function that applies a sine function to the current time (in milliseconds), using it to modulate the background color between amber and yellow.  It calls \code{COMMON.endFrame} at the end so that another animation frame is always requested, unless a WebGL error occurs.
