...
\item Preliminaries
\begin{enumerate}
\verbiage{A Brief History of *GL}{Describes WebGL's motivation, ancestry (OpenGL, OpenGL ES), and rapid growth.  Also briefly mentions impediments at the time of writing (security concerns and IE support).}
...
\begin{enumerate}
\verbiage{Distance Blur}{Depth of field via post-processing; also mentions a jitter-based technique} % Real-time Depth-of-Field (ShaderX 5) -- there also seem to be some dx11 demos for this (ATI's ladybug)
\verbiage{Ambient Occlusion}{Screen-space ambient occlusion}
\verbiage{Shadow Maps}{Old-school PCF shadow mapping} % Uses PCF to soften edges
\verbiage{Heat Shimmer}{Using the fragment shader to create a ``heat wave'' effect}
\rrecipe{Recipe 6: Desert Scene}{Rotates the camera around a static desert scene that uses all the techniques discussed in this chapter}
\end{enumerate}

\item Texture Tricks, Part I
\begin{enumerate}
\verbiage{Point Sprites}{Show how to use point sprites and set their size dynamically}
\verbiage{Derivatives Extension}{Shows how fragment shaders can smooth the edges of procedural patterns using OES\_standard\_derivatives}
\rrecipe{Recipe 7: Tube Sparks}{Fly through a tube with a stripe pattern, with embers falling onto the floor; animate fireworks in the vertex shader, rendered with additive blending}
\end{enumerate}

\item Texture Tricks, Part II
\begin{enumerate}
\verbiage{Reflection and Refraction with Cubemaps}{Classic, simple use of cubemaps} % refraction too?
\verbiage{Normal Mapping}{Bumps and dimples in tangent space} % external references: procedural bumping
\verbiage{Using the WebCam}{Capturing video in real time and using it as a WebGL texture} % http://dev.opera.com/static/articles/2012/webgl-postprocessing/webgl-pp/texImage2D.html
\verbiage{Anisotropic Filtering}{High-quality rendering of nearly edge-on geometry using WEBKIT\_EXT\_texture\_filter\_anisotropic}
\rrecipe{Recipe 8: Movie Theatre}{Displays a dimpled glass ball that refracts the webcam image}
\end{enumerate}
...
\rrecipe{Recipe 10: Set Dresser}{Place objects and spotlights into a scene} % might include uberlight, projection textures, and mirrors
\end{enumerate}

\item Beyond Giza: Related Libraries
\begin{enumerate}
...
%\verbiage{Parallax Mapping}{Improved bump mapping}
